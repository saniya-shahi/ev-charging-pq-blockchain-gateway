...
def register_user(name: str, password: str, mobile: str, pin: str, zone_code: str):
    ...
    with state_lock:
        ...
        # Prevent duplicate accounts with same mobile from race/misuse.
        existing_uid = next((k for k, u in users.items() if u["mobile"] == mobile), None)
        if existing_uid:
            existing = users[existing_uid]
            return {
                "UID": existing_uid,
                "VMID": existing["vmid"],
                "zone_code": existing["zone_code"],
                "message": "Mobile already registered; returning existing account",
            }
...
def register_franchise(name: str, password: str, zone_code: str, initial_balance: float = 0.0):
    ...
    if initial_balance < 0:
        return {"error": "initial_balance must be >= 0"}
    ...
    with state_lock:
        ...
        # Avoid duplicate station identity in the same zone.
        duplicate_fid = next(
            (
                k for k, f in franchises.items()
                if f["name"].strip().lower() == name.strip().lower() and f["zone_code"] == zone_code
            ),
            None,
        )
        if duplicate_fid:
            existing = franchises[duplicate_fid]
            return {
                "FID": duplicate_fid,
                "zone_code": existing["zone_code"],
                "message": "Franchise already exists in this zone; returning existing record",
            }
...
def process_transaction(encrypted_credential: str, amount: float, fid: str):
    ...
    with state_lock:
        # Step 2: Validate user, PIN, balance, franchise atomically.
        uid = next((k for k, u in users.items() if u["vmid"] == vmid), None)
        if not uid:
        ...
        # Step 3: Transfer funds under lock to prevent double-spend race.
        ts = time.time()
        user["balance"]            -= amount
        franchises[fid]["balance"] += amount
        print("[GRID] Balances updated")

        # Step 4: Build and record transaction block under same lock.
        ...
        # Step 5: Optional hardware failure simulation toggle.
        if hardware_failure_mode:
...
@app.get("/hardware_failure_mode")
def get_hardware_failure_mode():
    with state_lock:
        return {"enabled": hardware_failure_mode}


@app.post("/hardware_failure_mode")
def set_hardware_failure_mode(enabled: bool):
    global hardware_failure_mode
    with state_lock:
        hardware_failure_mode = enabled
        return {"enabled": hardware_failure_mode}
...
def reset_state(confirm: bool = False):
    """
    Demo-only helper to clear in-memory state between test/demo runs.
    """
    if not confirm:
        return {"error": "Pass confirm=true to reset state."}
    with state_lock:
        users.clear()
        franchises.clear()
        blockchain.chain = [blockchain.create_genesis_block()]
        return {"status": "State reset complete"}
